...
// Orisec LCD: 128 display units wide, 4 lines.
// Character widths per font size index in display units:
...
function renderLcd(canvas, raw, time) {
  ...
  // Parse into segments per line. Each segment is independently positioned.
  // Segment: { x: number (units), chars: [{ch,inv,fs}], align: str }
  // Or progress: { progress:true, value, max, barWidth }
  const lines = [[], [], [], []];
  let cur = -1, inverted = false, fontSize = 0, cursorX = 0;
  let seg = null; // current segment being built
  ...
  while (i < raw.length) {
    ...
    if (b === 17) {
      ...
      // Pad-right: flush current segment, reposition cursor
      i++;
      if (cur >= 0) {
        flush();
        cursorX = raw[i];
      ...
      // Center: start a centered segment
      flush();
  ...
  for (let ln = 0; ln < 4; ln++) {
    ...
    for (const s of segs) {
      ...
      // Measure each character's actual rendered width
      const widths = [];
      let totalMeasured = 0;
      ...
      // Use FONT_MULT only for alignment positioning (center/right)
      let xPx;
      if (s.align === "center") {
      ...
      // Clear segment area before drawing
      ctx.fillStyle = LCD_BG;
